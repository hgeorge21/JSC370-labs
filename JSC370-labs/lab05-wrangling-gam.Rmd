...
met[, temp_cat := fifelse(state_temperature <20, "low-temp",
                          fifelse(state_temperature<25,"mid-temp","high_temp"))]
```
...
The second cubic spline regression is better because the R-squared is higher while it does not overfit the data.
```{r}
...
```{r}
gam_mod <- gam(temp_50 ~ s(wind.sp_50, bs="cr", k=10), data=station_ave)
summary(gam_mod)
plot(gam_mod)
```
...
-   Paste the link to the github repo you created [here](https://github.com/hgeorge21/JSC370-labs)
